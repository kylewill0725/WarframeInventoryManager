...
def try_read_letters(img: np.ndarray, failed_bounds):
    ...
    for b in letter_bounds:
        cropped_image, border_size = add_border(NdImage.crop(cropped_word, (b[1], b[0], b[3], b[2])), multiplier=1.1, pixels=10)
...
def undo_word_wrap(img: np.ndarray):
    ...
    for j in range(len(imgs)):
        ...
        imgs[j] = NdImage.crop(
            i,
            (0, max(0, min_x - (BBoxes.WORD if j == 0 else 0))
...
def get_item_image_list(inv: InventoryImageInfo):
    ...
    for y in range(inv.row_count):
        for x in range(inv.column_count):
            if x + y * inv.column_count > 2 - 1:  # Skipping the first 0 items
